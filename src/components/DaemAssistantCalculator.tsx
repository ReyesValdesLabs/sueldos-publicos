...
function NumberField({ id, label, value, onChange, min = 0, max, suffix, help, error, onInfo }: { id: string; label: string; value: number; onChange: (value: number) => void; min?: number; max?: number; suffix?: string; help?: string; error?: string; onInfo?: () => void }) {
  ...
  return <div className="field-group"><div className="field-label-row"><Label htmlFor={id}>{label}</Label>{onInfo && <button type="button" className="field-info-button" onClick={onInfo} aria-label={`Más información sobre ${label}`}><CircleHelp size={15} /> Cómo obtenerlo</button>}</div><div className="relative"><Input id={id} type={money ? "text" : "number"} inputMode={money ? "numeric" : undefined} min={money ? undefined : min} max={money ? undefined : max} value={display} onInput={(event) => handleInput(event.currentTarget.value)} onBlur={() => setDraft(null)} aria-describedby={describedBy} aria-invalid={Boolean(error)} className={`form-control${suffix ? (money ? " form-control--suffix" : " form-control--number-suffix") : ""}`} />{suffix && <span className={`field-suffix pointer-events-none absolute top-1/2 -translate-y-1/2 text-sm font-medium text-muted-foreground${money ? "" : " field-suffix--number"}`}>{suffix}</span>}</div>{help && <p id={`${id}-help`} className="field-help">{help}</p>}{error && <p id={`${id}-error`} className="field-error" role="alert">{error}</p>}</div>;
}
...
export default function DaemAssistantCalculator({ embedded = false }: { embedded?: boolean }) {
  ...
  const minimumIncomeForHours = calculateDaemMinimumIncome(input.weeklyHours, input.minimumIncomeAgeBracket);
  ...
  return <section id={embedded ? undefined : "calculadora"} className="scroll-mt-24" aria-labelledby="daem-calculator-title">
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_21rem]">
      ...
      <aside className="order-first block print:hidden lg:order-none" aria-label="Información de alcance"><div className="space-y-4 lg:sticky lg:top-24"><div className="rounded-2xl border border-primary/15 bg-primary/5 p-5 text-sm"><div className="flex items-center gap-2 font-bold text-primary"><FileText size={17} /> Alcance acotado</div><p className="mt-2 leading-6 text-muted-foreground">Contrato DAEM/DEM en escuela o liceo municipal. SLEP, VTF, JUNJI e Integra usan reglas diferentes.</p></div></div></aside>
    </div>
